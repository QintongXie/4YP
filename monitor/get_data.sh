...
for model_file in "${model_dir}"/mlp_new.py; do
    ...
    for ((n=1; n<=1; n++)); do
        ...
        output_file="./data0416/${model_name}_cpu_log2_${n}.txt"
        echo "special python3 ${model_file}, times: ${n}"
        
        # Run the performance monitoring command and save the output for log2
        # python3 "${model_file}"
        perf stat -I 1000 -e cycles,instructions,cache-references,cache-misses,branches,branch-misses /home/univ5010/anaconda3/bin/python3 "${model_file}" > "${output_file}" 2>&1
    done
done
